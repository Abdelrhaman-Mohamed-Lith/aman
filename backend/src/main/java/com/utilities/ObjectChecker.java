...
public class ObjectChecker {
    ...
    public static boolean areEqual(Object object1, Object object2) {
        ...
        if (isAnyEmptyOrNull(object1, object2))
            return false;
        if (object1 instanceof BaseEntity e1 && object2 instanceof BaseEntity e2)
            return areEqual(e1.getCode(), e2.getCode());
        if (object1 instanceof Enum<?> || object2 instanceof Enum<?>)
    ...
    public static String toStringOrEmpty(Object value) {
        if (value == null)
            return "";
        String val = Translator.translate(value.toString());
    ...
    public static <T> List<String> translateList(T... values) {
        ...
        for (int i = 0; i < values.length; i++) {
            list.add(Translator.translate(values[i].toString()));
        }
        return list;
    }
    ...
    public static List<String> convertFromEnumListToStringList(List<? extends Object> items) {
        List<String> response = new ArrayList<>();
        for (Object item : items) {
            response.add(toStringOrEmpty(item));
        }
        return response;
    }

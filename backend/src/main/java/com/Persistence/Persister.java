...
public class Persister {
    ...
    public static Result saveOrUpdate(IEntity obj) {
        ...
        execute((r) -> {
            ...
            if (obj instanceof BaseEntity baseEntity) {
                ...
                if (byCode > 0)
                    result.failure("This code {0} already used", baseEntity.getCode());
            }
...
//                ((IEntity) obj).postCommit(r);
    ...
    public static <T> List<T> list(Class<T> klass, String condition, Map<String, Object> params) {
        ...
        return (List<T>) ObjectChecker.firstNotNull(wrapper.getObject(), new ArrayList<>());
    }

    public static <T> List<T> list(Class<T> klass, String condition, Map<String, Object> params, PaginationData paginationData) {
        ObjectWrapper<List<T>> wrapper = new ObjectWrapper<>();
        execute(r -> {
            Query<T> query = session.createQuery("From " + klass.getSimpleName() + " " + ObjectChecker.toStringOrEmpty(condition));
    ...
    public static <T> List<T> searchFor(String sqlString) {
        ...
        if (ObjectChecker.isEmptyOrZeroOrNull(list)) return null;
        return list;
    }
    ...
    private static <T> T getSingleResult(String sqlString, Map<String, Object> params, Function<String, Query> queryCreator) {
        Result result = new Result();
